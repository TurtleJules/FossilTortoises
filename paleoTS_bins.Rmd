...
hist(SpeciesMean$meanCL)

HistCLSpecies <- SpeciesMean %>%
  filter( !is.na(meanCL)) %>%
  ggplot(aes(meanCL)) + geom_histogram(binwidth=10, col="black", fill="gray") + facet_wrap(~EpochBins, scales="free_x")+
  theme_classic()+ 
  #theme_gray() +
...
HistEpoch <- SpeciesMean %>%
  filter( !is.na(meanCL)) %>%
  ggplot(aes(MeanBins)) + geom_histogram(bins=0.000001)
...
sumTortoises <- sumTort %>%
  ...
  select(Taxon, MeanBins, n, meanCL, sdCL) %>% #, EpochBins
#  group_by(Taxon) %>%
#  merge(Bins)
  bind_rows(SpeciesModern)

#sumTortoises[is.na(sumTortoises)] <- 5e-07
...
paleoSpec <-as.paleoTS(SpecPaleo$mm, SpecPaleo$vv, SpecPaleo$nn, SpecPaleo$tt, MM = NULL, genpars = NULL, label = "Testudinidae body size evolution mode, Holocene-Upper Miocene, CL, without Island species", reset.time=TRUE)
#paleoPPECL
plot(paleoSpec)
