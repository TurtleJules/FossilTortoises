...
# paleoTS Plot with the following bins:
...
```{r}
library(paleoTS)
paleoPPCL <-as.paleoTS(PPCL$mm, PPCL$vv, PPCL$nn, PPCL$tt, MM = NULL, genpars = NULL, label = "Testudinidae body size evolution mode, Holocene-Upper Miocene, CL")
paleoPPCL
plot(paleoPPCL)
...
PPECL <- bind_rows(ExTort, PPCL)

PPECL <- PPECL %>%
  filter(tt !=0) %>%
  arrange(tt)

paleoPPECL <-as.paleoTS(PPECL$mm, PPECL$vv, PPECL$nn, PPECL$tt, MM = NULL, genpars = NULL, label = "Testudinidae body size evolution mode, Holocene-Upper Miocene, CL")
paleoPPECL
plot(paleoPPECL)

fit3models(paleoPPECL, silent=FALSE, method="AD", pool=FALSE)
