...
FossOcc <- FossilOccurrences %>%
  ...
  select(Locality, Country, Latitude, Longitude, Age, Genus, Taxon) %>%
#  merge(tidyCL) %>%
  group_by(Locality) %>%
...
mapOc <- FossOcc %>%
  ...
  mapWorld +
  theme_classic() +
  geom_point(data=FossOcc, (aes(as.numeric(Longitude), Latitude))) +
  geom_point(data=CL,(aes(Longitude, Latitude, colour="o"))) #+
...
######HIER WEITER!!!!######
availableCL <- length(unique(CL$Locality))
availableOccurrences <- length(unique(FossOcc$Locality))

Comparison <- setdiff(CL, FossOcc)
comparison <- length(Comparison$Locality)

com <- setdiff(availableCL, Comparison)
...
MapCL <- tidyCL %>%
  filter(Age < 11.000) %>%
  filter(Latitude != "-") %>%
  dplyr::select(Genus, Taxon, Latitude, Longitude, Country, CL, PL, Epoch) %>%
  group_by(Latitude) %>%
  mutate(count= n())
