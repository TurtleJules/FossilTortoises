...
scatter <- testRatio %>%
  ...
  ggplot(aes(Age, CL)) + geom_jitter(aes(shape=Insularity, colour= Con)) +
  theme_classic() +
...
plot(SACFossOcSL,xlab="Localities",ylab="Richness", xvar="individuals", ci.type="line", ci.lty=2, ci.col="grey", col="deepskyblue4", lwd=2 )
```
...
# #add tukey letter: differences between groups
# Model4 <- aov(CL~EpochBins, data=PleiPlioCL)
# tHSD <- TukeyHSD(Model4, ordered = FALSE, conf.level = 0.95)
# plot(tHSD , las=1 , col="brown" )
# 
...
paleoGenAll <-as.paleoTS(GenusPaleo$mm, GenusPaleo$vv, GenusPaleo$nn, GenusPaleo$tt, MM = NULL, genpars = NULL, label = "Testudinidae body size evolution mode, Holocene-Upper Miocene, genus mean CL", reset.time=TRUE)
...
GenusMean <- PleiPlioCL %>%
  dplyr::filter(EpochBins != "Modern") %>%
  merge(Continents) %>%
  dplyr::filter(Con== "Asia" | Con == "Europe" & Island == "y") %>%
  dplyr::filter(!is.na(CL)) %>%
  ungroup() %>%
  dplyr::group_by(EpochBins, Genus) %>%
  dplyr::summarise(meanCL = mean(CL), n=n()) %>%
