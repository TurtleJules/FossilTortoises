...
kable(BINS, caption="Time bins with age range, mean age, epoch name and corresponding sample sizes (on individual, species and genus level)")
...
```{r "Boxplots of each genus per time bin", echo=FALSE}

IndGenera <- PleiPlioCL %>%
  group_by(Genus, MeanBins) %>%
  filter(!is.na(CL)) %>%
  ggplot(aes(MeanBins, CL, colour=Genus)) +
  theme_classic() +
  geom_boxplot()# +
 # facet_wrap(~.MeanBins)
