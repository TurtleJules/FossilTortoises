...
```
```{r "normal distribution, echo=FALSE", fig.cap="Data is not normally distributed."}
...
modern < fossil (`r ModernFossil$p.value`)
...
```
Wilcoxon Rank Sum Test (unpaired data): 
modern insular < fossil insular (`r ModernFossilIsland$p.value`)
modern continental < fossil continental (`r ModernFossilCon$p.value`)
...
Wilcoxon Rank Sum Test (unpaired data): 
continental < insular (`r InsularContinental$p.value`)
...
Wilcoxon Rank Sum Test (unpaired data): 
Continent means differ (`r ContinentsCompared$p.value`)
