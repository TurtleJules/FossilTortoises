...
# EpochBins <- as.vector(c("Modern", "Upper Pleistocene", "Middle Pleistocene", "Lower Pleistocene", "Upper Pliocene", "Lower Pliocene", "Upper Miocene", "Middle Miocene", "Lower Miocene", "Oligocene and Eocene"))
# 
# MeanBins <- as.vector(c((0+0.0117)/2, (0.0117+0.126)/2, (0.126+0.781)/2, (0.781+2.588)/2,(2.588+3.6)/2, (3.6+5.332)/2, (5.332+11.608)/2, (11.608+15.97)/2, (15.97+23.03)/2, (23.03+56.0)/2 ))
# 
# 
# bin <- as.vector(unique(Bins$bin))
# 
...
# #general statistics#
# Slength=length(StatsAll$CL)
# Smin=min(StatsAll$CL)
# Smax=max(StatsAll$CL)
# Svar=var(StatsAll$CL)
# Smean=mean(StatsAll$CL) 
# Slogmean=(mean(log10(StatsAll$CL)))
# Smedian=median(StatsAll$CL) 
# SMode=mode(StatsAll$CL)
# SlogMode=mode(log10(PleiPlioCL$CL))
# Slogmedian=(median(log10(StatsAll$CL)))
# Sskew=skewness(StatsAll$CL)
# Slogskew=skewness(log10(StatsAll$CL)) 
# Skurtosi=kurtosis(StatsAll$CL)
# Slogkurtosi=kurtosis(log10(StatsAll$CL))
# SizeStatsAll <- data.frame(Slength, Smin, Smax, Svar, Smean, Slogmean, Smedian, Slogmedian, SMode, SlogMode, Sskew, Slogskew, Skurtosi, Slogkurtosi)
# kable(SizeStatsAll)
# #write.csv(SizeStats,file="SizeStats.csv") 
...
Stats <- bind_rows(StatsAll, StatsBins, StatsIsland, StatsContinents) %>%
  select(-EpochBins, -Island, -Con)
...
Stats$Variable <- c("all", "Modern", "Upper Pleistocene", "Middle Pleistocene", "Lower Pleistocene", "Upper Pliocene", "Lower Pliocene", "Upper Miocene", "Middle Miocene", "Lower Miocene", "Oligocene and Eocene", "continental", "insular", "Africa", "America", "Asia", "Europe")

kable(Stats, caption="General statistics of body size data: all, per time bin, insular and continental")
...
```{r "Boxplots of each genus per time bin", echo=FALSE, warning=FALSE, fig.cap="Boxplots of generic mean CL per time bin, including mean and sd CL for each genus (as pointrange)."}
...
## play with time bins: no bins

```{r "paleoTS with different time bins, no bins", echo=FALSE}
...
paleoGenAll <-as.paleoTS(GenusPaleoAll$mm, GenusPaleoAll$vv, GenusPaleoAll$nn, GenusPaleoAll$tt, MM = NULL, genpars = NULL, label = "Testudinidae body size evolution mode, Holocene-Upper Miocene, genus mean CL", reset.time=TRUE)
...
kable(PaleoGenAllFit, caption="Model-fitting results for testudinidae, genera, only insular species")

```

\newpage

```{r "Play around with time bins", echo=FALSE}

Alternative <- PleiPlio
...
paleoAlter <-as.paleoTS(paleoAlternative$mm, paleoAlternative$vv, paleoAlternative$nn, paleoAlternative$tt, MM = NULL, genpars = NULL, label = "Testudinidae body size evolution mode, Holocene-Upper Miocene, genus mean CL", reset.time=TRUE)
...
```{r "Play around with time bins, species level", echo=FALSE}
...
SpeciesAlt <-as.paleoTS(SpeciesAlter$mm, SpeciesAlter$vv, SpeciesAlter$nn, SpeciesAlter$tt, MM = NULL, genpars = NULL, label = "Testudinidae body size evolution mode, Holocene-Upper Miocene, genus mean CL", reset.time=TRUE)
...
kable(SpeciesAltFit, caption="Model-fitting results for testudinidae, equal time bins, individuals")

```

\newpage

```{r "Play around with time bins, generic level", echo=FALSE}
...
GenusAlt <-as.paleoTS(GenusAlter$mm, GenusAlter$vv, GenusAlter$nn, GenusAlter$tt, MM = NULL, genpars = NULL, label = "Testudinidae body size evolution mode, Holocene-Upper Miocene, genus mean CL", reset.time=TRUE)
...
kable(GenusAltFit, caption="Model-fitting results for testudinidae, equal time bins, individuals")
...
```{r "Play around with larger time bins, generic level", echo=FALSE}
...
```{r "paleoTS, individuals, Africa", echo=FALSE, message=FALSE, fig.cap="individuals, excluding continental species"}
PPCLCon <- PleiPlioCL %>%
...
```{r "paleoTS, individuals, Europe", echo=FALSE, message=FALSE, fig.cap="individuals, excluding continental species"}
...
```{r "paleoTS, individuals, America", echo=FALSE, message=FALSE, fig.cap="individuals, excluding continental species"}
...
```{r "Model-fitting, individuals, America, table", echo=FALSE}
...
```{r "paleoTS, individuals, Asia", echo=FALSE, message=FALSE, fig.cap="individuals, excluding continental species"}
