...
kable(Stats, caption="General statistics of body size data: all, per time bin, insular and continental, per continent")
...
PaleoPCCLFit <- (fit4models(paleoPPCL, silent=FALSE, method="AD", pool=FALSE))
```
...
PaleoSpecFit <- (fit4models(paleoSpec, silent=FALSE, method="AD", pool=FALSE))
```
...
PaleoGenFit <- (fit4models(paleoGen, silent=FALSE, method="AD", pool=FALSE))
```
...
PaleoPCCLIsFit <- (fit4models(paleoPPCLIs, silent=FALSE, method="AD", pool=FALSE))
```
...
PaleoSpecIsFit <- (fit4models(paleoSpecIs, silent=FALSE, method="AD", pool=FALSE))
```
...
PaleoGenIsFit <- (fit4models(paleoGenIs, silent=FALSE, method="AD", pool=FALSE))
```
...
PaleoEuropeConFit <- (fit4models(paleoEuropeCon, silent=FALSE, method="AD", pool=FALSE))
```
...
```{r "paleoTS, individuals, America", echo=FALSE, message=FALSE, fig.cap="America, individuals"}
...
AmericaCon[is.na(AmericaCon)]<-0 #subset NAs with O for n=1
...
AmericaCon <- bind_rows(SumTortCon,AmericaCon) #ExTort, 

combiCLCon <- AmericaCon %>%
  dplyr::filter(tt==0.0000005) %>%   #maybe find a way to automatically filter double tt values...
  mutate(nx = nn*mm) %>%
  mutate(mmall=sum(nx)/sum(nn)) %>%
  mutate(SD=sqrt(nx), d=mm-mmall) %>%
...
kable(AmCon, caption="paleoTS object (mm= mean CL, nn = sample size, vv = variance (CL), tt = Age)")

#library(paleoTS)
paleoAmCon <-as.paleoTS(AmCon$mm, AmCon$vv, AmCon$nn, AmCon$tt, MM = NULL, genpars = NULL, label = "Testudinidae body size evolution mode, America")
#paleoPPCL
plot(paleoAmCon)
```
...
PaleoAmConFit <- (fit4models(paleoAmCon, silent=FALSE, method="AD", pool=FALSE))
```
...
kable(PaleoAmConFit, caption="Model-fitting results for testudinidae, individuals, America")
...
PaleoEurasiaConFit <- (fit4models(paleoEurasiaCon, silent=FALSE, method="AD", pool=FALSE))
```
