...
EpochBins <- as.vector(c("Modern", "Upper Pleistocene", "Middle Pleistocene", "Lower Pleistocene", "Upper Pliocene", "Lower Pliocene", "Upper Miocene"))


MeanBins <- as.vector(c((0+0.0117)/2, (0.0117+0.126)/2, (0.126+0.781)/2, (0.781+2.588)/2,(2.588+3.6)/2, (3.6+5.332)/2, (5.332+11.608)/2))
...
```{r "Histograms of body size data", echo=FALSE, fig.cap="Distribution of body size data"}
...
```{r "Histograms of body size data", echo=FALSE, fig.cap="Distribution of body size data per time bin"}



HistCLBins <- testRatio %>%
  filter( !is.na(CL)) %>%
  mutate(CL=log(CL)) %>%
#  filter(EpochBins != "Modern") %>%
  ggplot(aes(CL)) + geom_histogram( col="black", fill="gray") + 
...
```{r "Histograms of body size data", echo=FALSE, fig.cap="Distribution of body site data of continental (n) and insular(y) species."}



HistCLIsland <- testRatio %>%
  filter( !is.na(CL)) %>%
  mutate(CL=log(CL)) %>%
#  filter(EpochBins != "Modern") %>%
  ggplot(aes(CL)) + geom_histogram( col="black", fill="gray") + 
...
```{r "Boxplot continental vs. insular, genera summarised", echo=FALSE, fig.cap="Boxplot continental vs. insular, genera summarised"}
Island <- testRatio %>%
  merge(BINS) %>%
  ...
  filter(!is.na(CL)) %>%
  group_by(Genus, Island, Continent, Age) %>%
  summarise(GenusMean=mean(CL), GenusSD=sd(CL), n=n()) 
...
gray.colors(8, start = 0.5, end = 0.6, gamma = 2.2, alpha = NULL)
...
IslandBoxGenus <- Island %>%
  dplyr::select(GenusMean, Island, Continent, Age, Genus) %>%
  group_by(Genus, Island) %>%
  dplyr::filter(!is.na(GenusMean)) %>%
  summarise(meanCL=mean(GenusMean)) %>%
#  summarise(meanCL=mean(meanCL)) %>%
  ggplot(aes(Island, meanCL)) + geom_boxplot() + 
  geom_pointrange(data=Island, position="jitter", aes(x=Island, y=GenusMean,  colour= Continent , ymin = GenusMean-GenusSD, ymax = GenusMean+GenusSD)) +   #colour=Genus,
#  geom_pointrange(aes(colour=Continent, shape=Epoch)) +
#  facet_grid(.~Continent) +
  theme_classic() + # for white background 
#  stat_summary(fun.data = give.n, geom = "text") +
 # xlab("Sex") + ylab("Carapace length [mm]") +
...
```{r "Boxplot continental vs. insular", echo=FALSE}
