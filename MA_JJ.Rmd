...
############HIER WEITER!!!!##################
AltMeanBins <- as.vector(c((0:100.000)/2))
# need a function a+b/2
# a=(0:100.000)
# b=(0.5:100.000)
...
# 
# 
# AltBinsGenera <- Alternative %>%
#   select(bin, Genus) %>%
#   group_by(bin, Genus) %>%
#   summarise(nGenera=n()) %>%
#   summarise(nGenera=n())
# 
# Altbin <- as.vector(unique(AltBins$bin))
# 
# 
# 
# AltBINS <- data.frame(Altbin, AltMeanBins) %>%
#   merge(AltBins) %>%
#   merge(AltBinsSpecies) %>%
#   merge(AltBinsGenera) %>%
#   arrange(MeanBins)
